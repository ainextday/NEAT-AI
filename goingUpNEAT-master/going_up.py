...
# กำหนด Path ของโฟลเดอร์ที่เก็บไฟล์ sprites และ audio
Path = os.path.dirname(os.path.abspath(__file__))

# คำนวณตำแหน่งของพื้นหลัง
bg_y = -22500 + win_obj.current_h

# โหลดภาพ platform
pl_art = [
    pg.image.load(os.path.join(Path, "sprites", "platform.png")).convert(),
    pg.image.load(os.path.join(Path, "sprites", "platform_red.png")).convert(),
    pg.image.load(os.path.join(Path, "sprites", "platform_blue.png")).convert()
]

# โหลดภาพพื้นหลัง และทำการปรับขนาด
bg_art = pg.transform.scale(
    pg.image.load(os.path.join(Path, "sprites", "bg.png")).convert(),
    (scr_size, 22500)
)
...
class Blob:
    ...
    def __init__(self, x, y):
        ...
        self.jump = False
        self.gravity = 0
        # แก้ไขการโหลดภาพ blob_1.png
        self.img = pg.transform.scale(
            pg.image.load(os.path.join(Path, "sprites", "blob_1.png")).convert_alpha(),
            (self.size, self.size)
        )
        self.jump_force = 0
